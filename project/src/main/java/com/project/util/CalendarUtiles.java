...
public class CalendarUtiles {

    public static List<CalendarVM> getDayOfMonthFormat(int year, int month) {

        List<CalendarVM> calendarVMList = new ArrayList<>();
        ...
        int dayNum = 1;
        int nextDayNum = 1;
        for (int i = 0; i < daysOfFirstWeek - 1; i++)
            calendarVMList.add(new CalendarVM());
        for (int i = 0; i < daysOfMonth; i++) {
            CalendarVM calendarVM = new CalendarVM(year, month);
            calendarVM.setDay(nextDayNum++);
            calendarVMList.add(calendarVM);
        }
        return calendarVMList;
    }
