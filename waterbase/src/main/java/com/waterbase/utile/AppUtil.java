...
import com.waterbase.global.AppProcessInfo;
import com.waterbase.global.CPUInfo;
import com.waterbase.global.ProcessInfo;
...
//	        info = context.getPackageManager().getPackageInfo(packageName,PackageManager.GET_ACTIVITIES);
    ...
    public static List<AppProcessInfo> getRunningAppProcesses(Context context) {
        ...
        try {
            ...
            for (RunningAppProcessInfo appProcessInfo : appProcessList) {
                AppProcessInfo = new AppProcessInfo(appProcessInfo.processName, appProcessInfo.pid, appProcessInfo.uid);
                appInfo = getApplicationInfo(context, appProcessInfo.processName);
                if (appInfo != null) {
                    Drawable icon = appInfo.loadIcon(packageManager);
                    String appName = appInfo.loadLabel(packageManager).toString();
                    AppProcessInfo.icon = icon;
                    AppProcessInfo.appName = appName;
                } else {
                    //:服务的命名
                    ...
                    AppProcessInfo.appName = appProcessInfo.processName;
                }

	    		/*PsRow psRow = getPsRow(appProcessInfo.processName);
	    		if(psRow!=null){
	    			AppProcessInfo.memory = psRow.mem;
	    		}*/

                ProcessInfo processInfo = getMemInfo(appProcessInfo.processName);
                AppProcessInfo.memory = processInfo.memory;
                AppProcessInfo.cpu = processInfo.cpu;
                AppProcessInfo.status = processInfo.status;
                AppProcessInfo.threadsCount = processInfo.threadsCount;
    ...
    public static ProcessInfo getMemInfo(String processName) {
        ...
        for (Iterator<String[]> iterator = mProcessList.iterator(); iterator.hasNext(); ) {
            ...
            //LogUtil.d(AppUtil.class, "##"+item[9]+",NAME:"+processNameTemp);
            if (processNameTemp != null && processNameTemp.equals(processName)) {
                //LogUtil.d(AppUtil.class, "##"+item[9]+","+process.memory);
                //Process ID
    ...
    public static ProcessInfo getMemInfo(int pid) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            //LogUtil.d(AppUtil.class, "##"+item[9]+",PID:"+tempPid);
    ...
    public static List<String[]> parseProcessRunningInfo(String info) {
        ...
        for (int i = 0; i < rows.length; i++) {
            tempString = rows[i];
            //LogUtil.d(AppUtil.class, tempString);
            if (tempString.indexOf("PID") == -1) {
                if (bIsProcInfo == true) {
                    ...
                    if (columns.length == Length_ProcStat) {
                        ...
                        //LogUtil.d(AppUtil.class, "#"+columns[9]+",PID:"+columns[0]);
